...
::: {.notes}
Various studies have been done on smart charging programs, but they all have small sample size and low EV ownership. Therefore, we aimed to conduct a large-scale survey with high EV ownership to study the heterogeneity of BEV owners' preferences.
:::
...
On the right side is a sample list of conjoint attributes. They represent the smart charging features of a program. We'll talk in details in later slides.
:::
...
::: {.notes}
But first, I want to explain what conjoint question is. In this apple example, you can only choose one set of the attributes, so if you pick one option, you pick all the attributes in it. In short, conjoint is about forcing people to make trade-offs.
:::
...
<img src="https://raw.githubusercontent.com/pingfan-hu/pingfanResources/main/bev/battery_smc/u_200_g_160_m_80.png" style="width: 400px; width: 100%; height: auto;"/>
:::
:::
:::
:::
...
::: {style="text-align: center;"}
<img src="https://raw.githubusercontent.com/pingfan-hu/pingfanResources/main/bev/battery_v2g/u_200_g_160_l_80.png" style="width: 400px; width: 100%; height: auto;"/>
:::
:::
:::
:::
...
::: {.notes}
This is the conjoint question design for V2G programs. V2G is short for Vehicle-to-Grid. The attributes are similar to SMC, but with some differences. For example, we have occurrence cash and monthly occurrence instead of override.
...
::: {.notes}
There are 6 SMC conjoint questions. In each question, participants are provided with 2 valid options and a "Not Interested" option. It's important to have this "Not Interested" option as a validation of the willingness of the participants.
:::
...
::: {.notes}
Likewise, there are 6 V2G conjoint questions as well.
:::
...
smc_attr_equiv %>%
  ...
  row_spec(5, bold = TRUE, color = "#739C69")
```
...
v2g_attr_equiv %>%
  ...
  kable_styling(font_size = 21) %>%
  row_spec(0, bold = TRUE) %>% 
  row_spec(c(1, 2), bold = TRUE, color = "#739C69")
```
...
::: {.notes}
Following the linearity of the sensitivity plots, there is a convenient way to reveal the equivalencies of enrollment increases. Here for each program, we start from its baseline. We increase each feature value individually, and see how much each one needs to be, in order to achieve a 5% enrollment increase. And here are our interpretations:
...
Also, guaranteed threshold is more important in SMC than in V2G, indicating range anxiety.

Finally, monetary incentives are valued more in V2G than in SMC. This indicates that BEV owners are willing to leverage their BEVs as a source of income.
:::
...
::: {.notes}
So, after analysis of all these individual features, we can combine them into programs. We call them "scenarios". Here for SMC we have 3 scenarios. They are flexibility, recurring cash, and one-time cash designated.
...
::: {.notes}
For V2G programs, recurring incentives are also more important than one-time, flexibility is compromised since ownrs are willing to use leverage their BEVs as a source of income. So the V2G programs are more incentive powered than SMC programs.
:::
